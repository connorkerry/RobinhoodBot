...
def show_plot(price, firstIndicator, secondIndicator, dates, label1="", label2="", symbol=""):
...
def get_equity_data():
    ...
    for key, value in holdings_data.items():
        ...
        equity_data[key]['type'] = value.get("type")
        labels.append(value.get("name"))
        print(value)
    fig1, ax1 = plt.subplots()
    ax1.pie(equity_data, labels=labels, autopct='%1.1f%%', shadow=True, startangle=90)
    ax1.axis('equal')
    plt.show(fig1)
